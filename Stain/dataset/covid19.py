...
class CornerCrop(object):
    def crop_corner(self, img):
        _, y, x = img.shape
        startx = self.corner_size
        starty = self.corner_size
        for i in range(0,startx):
            for j in range(0,starty):
                img[:, j, i] = random.random()*255

        for i in range(x-startx, x):
            for j in range(0, starty):
                img[:, j, i] = random.random()*255

        return img

    def __init__(self, corner_size=40):
        self.corner_size = corner_size

    def __call__(self, img):
        return self.crop_corner(img)

class Tofloat32(object):
    def __call__(self, img):
        return img.type(torch.float32)

class To0_255(object):
    def __call__(self, img):
        return img*255
...
class histeq(object):
    def histeq(self, im, nbr_bins=256):
        ...
        cdf = cdf / cdf[-1]  # 归一化，函数中使用累计分布函数的最后一个元素（下标为-1，目标是
        # 将其归一化到0-1范围 ）
...
if __name__ == '__main__':
    ...
    test_NIH_dataset = NIH_Dataset(
                 imgpath = os.path.join(thispath, "NIH", "images-224"),
                 csvpath=os.path.join(thispath, "NIH", "Data_Entry_2017.csv"),
                 transform=None,
                 data_aug=None,
                 nrows=None,
                 seed=0,
    ...
    Dataset_fin = Merge_Dataset((filtered_covid, filtered_NIH), seed=1000)
